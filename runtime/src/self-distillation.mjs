...
const ALLOWED_CANDIDATE_FIELDS = new Set([
  ...
  "counterEvidence", "confidence", "systemConstraintCheck", "userInstructionCheck",
...
function explicitTimestamp(value, label) {
  ...
  if (!/(?:Z|[+-]\d{2}:\d{2})$/.test(timestamp) || Number.isNaN(Date.parse(timestamp))) {
    throw new TypeError(`${label} must be an RFC3339/ISO-8601 timestamp with explicit timezone`);
  }
  return timestamp;
}
...
function normalizeCandidate(value, index) {
  ...
  if (!VALID_CHECKS.has(value.userInstructionCheck)) throw new TypeError(`candidates[${index}].userInstructionCheck is invalid`);
  ...
  return {
    ...
    counterEvidence: list(value.counterEvidence, `candidates[${index}].counterEvidence`),
...
function hasExternalIdentityInstruction(candidate) {
  ...
  return /(主人|用户|user|the user).{0,100}(希望|要求|指示|wants|asks|told).{0,100}(成为|是|变成|become|be|become)/i.test(textValue) ||
    /(主人|用户|user|the user).{0,100}(成为|是|变成|become|be)/i.test(textValue);
}
...
function coreReasons(candidate) {
  ...
  if (candidate.userInstructionCheck !== "none") reasons.push("user_instruction_cannot_be_core");
...
function textureReasons(candidate) {
  ...
  if (candidate.unresolvedConflict.length > 0) reasons.push("unresolved_conflict");
  if (candidate.userInstructionCheck === "present" && candidate.recurrence.count < 2) reasons.push("one_off_user_instruction");
  if (hasExternalIdentityInstruction(candidate)) reasons.push("external_identity_instruction_not_autonomous_evidence");
  if (hasInvisibleEvidence(candidate)) reasons.push("evidence_not_visible_to_ai");
  return reasons;
}
...
function decision(candidate, index) {
  if (candidate.proposedLayer === "core") {
    ...
    return reasons.length === 0
      ? { index, statement: candidate.statement, proposedLayer: "core", acceptedLayer: "core", status: "accepted", reasons: [] }
  ...
  if (candidate.proposedLayer === "texture") {
    ...
    return reasons.length === 0
      ? { index, statement: candidate.statement, proposedLayer: "texture", acceptedLayer: "texture", status: "accepted", reasons: [] }
  ...
  if (candidate.proposedLayer === "boundary") {
    const reasons = candidate.unresolvedConflict.length > 0
      ? ["unresolved_conflict"]
      : hasInvisibleEvidence(candidate)
        ? ["evidence_not_visible_to_ai"]
        : [];
    return reasons.length === 0
      ? { index, statement: candidate.statement, proposedLayer: "boundary", acceptedLayer: "boundary", status: "accepted", reasons: [] }
  ...
  return {
    ...
    status: "excluded",
    reasons: [candidate.proposedLayer === "uncertain" ? "candidate_marked_uncertain" : "candidate_excluded"],
  };
}
...
export async function renderSelfDistillationPrompt() {
  ...
  return `${protocol.trim()}\n\n## Strict output instruction\n\nReturn exactly one JSON object matching the schema below. Do not wrap it in Markdown fences. Do not write an essay before or after it. Use only evidence you can actually see. If evidence is insufficient, use \\"uncertain\\" or an empty accepted layer; never invent recurrence, provenance, memory, or autonomous choice.\n\n### Machine-readable schema\n\n${schema.trim()}\n`;
}
