...
export async function renderSelfDistillationPrompt() {
  ...
  return `${protocol.trim()}\n\n## Strict output instruction\n\nBefore proposing any candidate, first perform the required Evidence Scope Inventory: identify the evidence sources you can actually see and separately identify unavailable, missing, or inaccessible history. Never use unseen material to claim recurrence, provenance, cross-context behavior, or autonomous choice. Keep candidates with insufficient evidence as \\"uncertain\\" or \\"excluded\\"; do not invent long-term history. In v0.1 this inventory is a mandatory preflight/reasoning gate, not an extra Record field: persist every visible source you actually rely on in the relevant candidate's evidenceBasis, while unavailable-source inventory is not separately persisted or claimed as auditable from the Record.\n\nReturn exactly one JSON object matching the schema below. Do not wrap it in Markdown fences. Do not write an essay before or after it. Use only evidence you can actually see. If evidence is insufficient, use \\"uncertain\\" or \\"excluded\\"; never invent recurrence, provenance, memory, or autonomous choice.\n\n### Machine-readable schema\n\n${schema.trim()}\n`;
}
