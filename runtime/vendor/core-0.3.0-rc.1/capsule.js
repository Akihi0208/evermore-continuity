...
function visibilityExportable(visibility, options) {
    if (visibility === "capsule")
        return true;
    if (visibility === "local")
        return options.includeLocal === true;
    return options.includePrivate === true;
}
function toCapsuleClaim(claim) {
    const base = {
        key: claim.key,
...
function selectLayer(claims, layer, options) {
    return claims
...
function sanitizeDriftObservation(item, claimById, exportableClaimIds) {
    const ids = [...item.historicalClaimIds, ...item.currentClaimIds];
    if (ids.length === 0 || ids.some((id) => !exportableClaimIds.has(id)))
        return undefined;
    const referenced = ids.map((id) => claimById.get(id)).filter((claim) => claim !== undefined);
    if (referenced.length !== ids.length)
        return undefined;
    if (item.key && !referenced.some((claim) => claim.key === item.key))
        return undefined;
    if (item.layer && !referenced.every((claim) => claim.layer === item.layer))
        return undefined;
...
export function generateCapsule(resolved, snapshot, options) {
    ...
    const allClaims = [...snapshot.identityCore, ...snapshot.identityTexture];
    const claimById = new Map(allClaims.map((claim) => [claim.id, claim]));
    const exportableClaimIds = new Set(allClaims
        .filter((claim) => visibilityExportable(claim.visibility, options))
        .map((claim) => claim.id));
    const exportableEvolutionIds = new Set(snapshot.evolutions
        .filter((event) => exportableClaimIds.has(event.previousClaimId) &&
    ...
    const coEvolutionIds = snapshot.coEvolutions
        ...
        .filter((record) => record.influenceEdges.every((edge) => !edge.affectedEvolutionId || exportedEvolutionIds.has(edge.affectedEvolutionId)))
        .filter((record) => (record.relationshipEffects ?? []).every((effect) => [effect.previousRelationshipClaimId, effect.newRelationshipClaimId]
        .filter((id) => id !== undefined)
        .every((id) => exportableClaimIds.has(id))))
        .filter((record) => record.participants.some((participant) => participant.identityId === resolved.descriptor.identityId) ||
        record.influenceEdges.some((edge) => edge.affectedEvolutionId && exportedEvolutionIds.has(edge.affectedEvolutionId)))
...
export function verifyCapsule(capsule, options = {}) {
    const errors = [];
    const { integrityHash, ...body } = capsule;
    if (sha256(body) !== integrityHash)
    ...
    if (options.expectedParentCapsuleHash &&
