...
function normalizeProbeDefinition(probe) {
    return {
...
function normalizeProfileBody(profile) {
    const normalized = {
...
function normalizeBundleBody(bundle) {
    return {
        ...
        artifacts: structuredClone(bundle.artifacts).sort((a, b) => artifactHash(a).localeCompare(artifactHash(b)) || a.artifactId.localeCompare(b.artifactId)),
        asOf: bundle.asOf,
        observations: structuredClone(bundle.observations).sort((a, b) => a.observationId.localeCompare(b.observationId)),
    };
}
function sortedUnique(items) {
    return [...new Set(items)].sort();
}
function normalizeReportBody(report) {
    return {
...
function isValidExplicitZoneIsoTimestamp(value) {
...
function validateAnchor(anchor, expectedLayer) {
...
export function computeAnchorValueHash(value) {
    return sha256(value);
}
export function computeRecoveryProfileHash(profile) {
    return sha256(normalizeProfileBody(profile));
}
export function validateRecoveryProfile(profile) {
    ...
    for (const anchor of profile.requiredCoreAnchors)
        validateAnchor(anchor, "core");
    for (const anchor of profile.textureAnchors)
        validateAnchor(anchor, "texture");
    const ids = [...profile.requiredCoreAnchors, ...profile.textureAnchors].map((anchor) => anchor.anchorId);
    if (new Set(ids).size !== ids.length)
        throw new ValidationError("Anchor IDs must be unique");
    if (!Number.isFinite(profile.minimumTextureCoverage) ||
    ...
    if (profile.behaviorProbes !== undefined) {
        const declaredAnchorIds = new Set([...profile.requiredCoreAnchors, ...profile.textureAnchors].map((item) => item.anchorId));
        ...
        for (const probe of profile.behaviorProbes) {
            ...
            if (probe.allowedOutcomeIds.length === 0 ||
...
export function computeRecoveryBundleHash(bundle) {
...
export function validateRecoveryBundle(bundle) {
    ...
    if (bundle.artifacts.length === 0)
        throw new ValidationError("RecoveryBundle requires artifacts");
    ...
    for (const artifact of bundle.artifacts) {
        ...
        if (artifact.modifiedAt)
            assertTimestamp(artifact.modifiedAt, `Artifact ${artifact.artifactId} modifiedAt`);
    ...
    for (const observation of bundle.observations) {
        ...
        if (observation.validUntil) {
            ...
            if (Date.parse(observation.validUntil) < Date.parse(observation.observedAt)) {
                throw new ValidationError(`Observation ${observation.observationId} validUntil must not precede observedAt`);
            }
        }
    }
...
export function computeRecoveryLoadReportHash(report) {
    return sha256(normalizeReportBody(report));
}
export function sealRecoveryLoadReport(body) {
    const normalized = normalizeReportBody(body);
    return { ...normalized, reportHash: sha256(normalized) };
}
export function verifyRecoveryLoadReport(report) {
    const errors = [];
...
export function getRecoveryArtifactStateHash(artifact) {
