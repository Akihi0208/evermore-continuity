...
const FACT_CAPABLE_EVIDENCE = new Set([
    "direct_observation",
    "external_source",
]);
...
export function validateEvidence(record) {
    ...
    if (!record.sourceId.trim())
        throw new ValidationError("Evidence sourceId is required");
...
export function validateClaimProvenance(claim, evidenceById) {
    ...
    const evidence = claim.evidenceIds.map((id) => {
        ...
        if (!record)
            throw new ValidationError(`Claim ${claim.id} references missing evidence ${id}`);
        return record;
    });
    if (claim.claimKind === "fact") {
        const supported = evidence.some((record) => record.verified && FACT_CAPABLE_EVIDENCE.has(record.evidenceKind));
        if (!supported) {
            throw new ValidationError(`Fact claim ${claim.id} requires verified direct-observation or external-source evidence`);
        }
    }
}
