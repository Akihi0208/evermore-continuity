...
function assertTimestamp(value: string, label: string): void {
  if (Number.isNaN(Date.parse(value))) throw new ValidationError(`${label} must be an ISO timestamp`);
}
...
type SnapshotBody = Omit<LedgerSnapshot, "snapshotHash">;

function normalizedSnapshotBody(snapshot: SnapshotBody | LedgerSnapshot): SnapshotBody {
  return {
...
export function computeSnapshotHash(snapshot: SnapshotBody | LedgerSnapshot): string {
  return sha256(normalizedSnapshotBody(snapshot));
}

export function assertSnapshotIntegrity(snapshot: LedgerSnapshot): void {
...
export class ContinuityLedger {
  ...
  readonly #core = new Map<string, IdentityCoreClaim>();
  readonly #texture = new Map<string, IdentityTextureClaim>();
  readonly #evolutions = new Map<string, EvolutionEvent>();
  readonly #selfAcceptances = new Map<string, SelfAcceptanceRecord>();
  readonly #coEvolutions = new Map<string, CoEvolutionRecord>();
  readonly #evolutionTargets = new Set<string>();

  constructor(descriptor: IdentityDescriptor) {
    if (!descriptor.identityId.trim()) throw new ValidationError("identityId is required");
    if (!descriptor.lineageId.trim()) throw new ValidationError("lineageId is required");
  ...
  appendEvidence(record: EvidenceRecord): this {
    if (this.#evidence.has(record.id)) throw new DuplicateRecordError("Evidence", record.id);
    validateEvidence(record);
    this.#evidence.set(record.id, clone(record));
    return this;
  }

  appendClaim(claim: IdentityClaim): this {
    const claimId = claim.id;
    if (this.#claimById(claim.id)) throw new DuplicateRecordError("Claim", claim.id);
    this.#assertIdentity(claim.identityId, claim.lineageId, `Claim ${claim.id}`);
    if (!claim.key.trim()) throw new ValidationError(`Claim ${claim.id} requires a key`);
    ...
    if (claim.validUntil) assertTimestamp(claim.validUntil, `Claim ${claim.id} validUntil`);
    if (
      claim.validFrom &&
      claim.validUntil &&
    ...
    if (claim.driftWeight !== undefined && (
      !Number.isFinite(claim.driftWeight) || claim.driftWeight < 0 || claim.driftWeight > 1
    )) {
      throw new ValidationError(`Claim ${claim.id} driftWeight must be between 0 and 1`);
    }
    if (claim.layer === "core") {
      if (
        claim.stabilityProfile !== "slow" ||
      ...
      claim.stabilityProfile !== "adaptive" ||
      !["accepted_evolution_required", "observed_growth_with_review"].includes(
        claim.changePolicy,
      )
    ) {
    ...
    if (claim.layer === "core") this.#core.set(claim.id, clone(claim));
    else this.#texture.set(claim.id, clone(claim));
    return this;
  }

  appendEvolution(event: EvolutionEvent): this {
    if (this.#evolutions.has(event.id)) throw new DuplicateRecordError("Evolution", event.id);
    ...
    if (!previous) throw new ValidationError(`Evolution ${event.id} references missing previous claim`);
    if (!next) throw new ValidationError(`Evolution ${event.id} references missing new claim`);
  ...
  appendSelfAcceptance(record: SelfAcceptanceRecord): this {
    ...
    this.#assertEvidenceReferences(
      record.evidenceIds,
      `SelfAcceptance ${record.id}`,
    ...
    if (record.revision === 1) {
      ...
      if (
        previous.subjectIdentityId !== record.subjectIdentityId ||
  ...
  appendCoEvolution(record: CoEvolutionRecord): this {
    if (this.#coEvolutions.has(record.id)) throw new DuplicateRecordError("CoEvolution", record.id);
    ...
    for (const edge of record.influenceEdges) {
      ...
      if (!edge.description.trim()) throw new ValidationError(`Influence edge ${edge.id} requires a description`);
  ...
  #claimById(id: string): IdentityClaim | undefined {
    return this.#core.get(id) ?? this.#texture.get(id);
  }

  #assertIdentity(identityId: string, lineageId: string, label: string): void {
  ...
  #assertEvidenceReferences(ids: string[], label: string, requireOne: boolean): void {
    if (requireOne && ids.length === 0) throw new ValidationError(`${label} must cite evidence`);
  ...
  #chainContainsAccepted(start: SelfAcceptanceRecord): boolean {
    let current: SelfAcceptanceRecord | undefined = start;
