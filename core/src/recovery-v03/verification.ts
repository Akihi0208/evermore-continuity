...
import type {
  RecoveryDriftResult,
  RecoveryConflictResult,
  RecoveryEvaluationOptions,
  RecoveryOperationalObservation,
  RecoveryProbeDefinition,
  RecoveryProbeObservation,
  RecoveryProbeResult,
  RecoveryReportVerificationResult,
  RecoveryVerificationAdapterResult,
  RecoveryVerificationReport,
  RecoveryVerificationReportBody,
  RecoveryLoadReport,
  RecoveryProfile,
  RecoveryVerdict,
} from "./types.js";
...
function normalizeProbeResult(result: RecoveryProbeResult): RecoveryProbeResult {
  return {
...
function normalizeDriftResult(result: RecoveryDriftResult): RecoveryDriftResult {
  return {
...
function normalizeConflictResult(result: RecoveryConflictResult): RecoveryConflictResult {
  return {
...
function normalizeVerificationReportBody(
  report: RecoveryVerificationReportBody | RecoveryVerificationReport,
): RecoveryVerificationReportBody {
  return {
...
export function computeRecoveryVerificationReportHash(
  report: RecoveryVerificationReportBody | RecoveryVerificationReport,
): string {
  return sha256(normalizeVerificationReportBody(report));
}

export function sealRecoveryVerificationReport(
  body: RecoveryVerificationReportBody,
): RecoveryVerificationReport {
  const normalized = normalizeVerificationReportBody(body);
  return { ...normalized, reportHash: sha256(normalized) };
}

export function verifyRecoveryVerificationReport(
...
function observationMap(
  observations: RecoveryProbeObservation[],
): Map<string, RecoveryProbeObservation[]> {
  const grouped = new Map<string, RecoveryProbeObservation[]>();
...
function evaluateProbe(
  ...
  observations: RecoveryProbeObservation[] | undefined,
  exportableAnchorIds: ReadonlySet<string>,
): RecoveryProbeResult {
  ...
  const matchedAnchorIds = definition.anchorIds.filter(
    (anchorId) => cited.has(anchorId) && exportableAnchorIds.has(anchorId),
  );
...
function driftResults(options: RecoveryEvaluationOptions): RecoveryDriftResult[] {
...
function operationalWarnings(observations: RecoveryOperationalObservation[]): string[] {
  return sortedUnique(observations.map((item) => `operational:${item.domain}:${item.status}`));
}

function safeAnchorIds(profile: RecoveryProfile): Set<string> {
  return new Set(
    [...profile.requiredCoreAnchors, ...profile.textureAnchors]
      .filter((item) => item.visibility !== "private")
      .map((item) => item.anchorId),
  );
}

function keepSafe(items: string[], safeIds: ReadonlySet<string>): string[] {
  return items.filter((item) => safeIds.has(item));
}

function verdictFor(rejected: boolean, indeterminate: boolean): RecoveryVerdict {
...
export function evaluateRecovery(
  ...
  loadReport: RecoveryLoadReport,
  probeObservations: RecoveryProbeObservation[],
  options: RecoveryEvaluationOptions = {},
): RecoveryVerificationReport {
  ...
  const probeResults = definitions.map((definition) =>
    evaluateProbe(definition, observationsByProbe.get(definition.probeId), safeIds));
  for (const result of probeResults) {
    if (!result.critical) {
      if (result.status === "failed") warnings.push("noncritical_probe_failed");
      continue;
    }
...
export function serializeRecoveryVerification(
  report: RecoveryVerificationReport,
): RecoveryVerificationAdapterResult {
  const verification = verifyRecoveryVerificationReport(report);
  if (!verification.valid) throw new ValidationError("report_integrity_mismatch");
  return {
