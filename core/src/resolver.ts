...
function claimSort(a: IdentityClaim, b: IdentityClaim): number {
  return (
...
function eventSort(a: EvolutionEvent, b: EvolutionEvent): number {
  return a.timestamp.localeCompare(b.timestamp) || a.id.localeCompare(b.id);
}

function inWindow(claim: IdentityClaim, asOf: string): boolean {
  const time = Date.parse(asOf);
  if (time < Date.parse(claim.createdAt)) return false;
  if (claim.validFrom && time < Date.parse(claim.validFrom)) return false;
  if (claim.validUntil && time >= Date.parse(claim.validUntil)) return false;
  return true;
}

function identityFingerprint(lineageId: string, activeClaims: IdentityClaim[]): string {
...
interface AcceptanceResolution {
  status?: AcceptanceStatus;
  conflict?: AcceptanceResolutionConflict;
}

function acceptanceConflict(
...
function hasCycle(records: SelfAcceptanceRecord[]): string[] {
  const byId = new Map(records.map((record) => [record.id, record]));
  for (const record of records) {
    const path: string[] = [];
    const positions = new Map<string, number>();
    let current: SelfAcceptanceRecord | undefined = record;
    while (current) {
      const position = positions.get(current.id);
      if (position !== undefined) return path.slice(position).concat(current.id);
...
function resolveAcceptance(
  ...
  if (!allRoot) return acceptanceConflict(event, "acceptance_root_missing", [event.acceptanceId]);
  if (Date.parse(allRoot.recordedAt) > cutoff) return { status: "pending" };

  const eligible = snapshot.selfAcceptances.filter(
  ...
  if (cycleIds.length > 0) return acceptanceConflict(event, "acceptance_cycle", cycleIds);

  const children = new Map<string, SelfAcceptanceRecord[]>();
  for (const record of eligible) {
    if (!record.revisesAcceptanceId) continue;
  ...
  while (true) {
    ...
    if (nextRecords.length > 1) {
      return acceptanceConflict(
        event,
        "acceptance_branch",
        [current.id, ...nextRecords.map((item) => item.id)],
      );
    }
    ...
    if (next.status === "accepted") hasAccepted = true;
    current = next;
  }
  ...
  if (unreachable.length > 0) {
    return acceptanceConflict(
      ...
      unreachable.map((record) => record.id),
    );
  }
...
export function resolveIdentity(
  ...
  if (Number.isNaN(Date.parse(asOf))) throw new TypeError("asOf must be an ISO timestamp");
  const claims: IdentityClaim[] = [...snapshot.identityCore, ...snapshot.identityTexture];
  ...
  const pendingEvolutionIds: string[] = [];
  const rejectedEvolutionIds: string[] = [];
  const withdrawnEvolutionIds: string[] = [];
  const acceptedEvolutionIds: string[] = [];
  const acceptedEvolutionClaimIds: string[] = [];
  const ambiguousEvolutionIds: string[] = [];
  ...
  for (const timestamp of timestamps) {
    ...
    for (const event of eligibleEvents.filter((item) => item.timestamp === timestamp)) {
      ...
      if (!previous || !next || !inWindow(next, asOf)) continue;
      candidates.push({ event, previous, next });
    }

    const competingIds = new Set<string>();
    const consumersByPrevious = new Map<string, typeof candidates>();
...
export function hasSharedLineage(a: ResolvedIdentity, b: ResolvedIdentity): boolean {
