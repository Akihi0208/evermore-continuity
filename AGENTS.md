...
Help the user run the Personal Runtime without treating the repository as an automatic memory product. The usable alpha creates an encrypted local vault, exports a reviewed portable package, and renders a model-neutral handoff.
...
5. Export with `node runtime/bin/evermore.mjs export runtime-secrets/persona.evermore-vault.json`.
6. Verify the exported package, render the handoff, and ask the user to review the visible contents before sending it to another model.
...
- Do not commit anything under `runtime-secrets/` or any generated vault, portable package, or handoff.
